...
class PRoTOn(RecoveryProtocol):
    ...
    def run(self):
        ...
        while len(get_demand_edges(G, is_check_unsatisfied=True)) > 0:
            ...
            # -------------- 0. Monitor placement --------------
            # if self.config.protocol_monitor_placement == co.ProtocolMonitorPlacement.BUDGET_W_REPLACEMENT:
            #     monitors_map = mon.removing_monitor(G, monitors_map, self.config)
            ...
            # if self.config.protocol_monitor_placement in [co.ProtocolMonitorPlacement.STEP_BY_STEP, co.ProtocolMonitorPlacement.STEP_BY_STEP_INFINITE]:
            #     monitors = mon.original_monitoring_add(G, self.config)
